...
public class SignalKReceiver extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		// timed actions
		from("timer://declination?fixedRate=true&period=10000").process(new DeclinationProcessor()).to("log:nz.co.fortytwo.signalk.signalk.update?level=INFO").end();
		from("timer://wind?fixedRate=true&period=1000").process(new WindProcessor()).to("log:nz.co.fortytwo.signalk.signalk.update?level=INFO").end();
		from("timer://signalkAll?fixedRate=true&period=1000").process(new SignalkModelExportProcessor()).to("log:nz.co.fortytwo.signalk.signalk.signalkAll?level=INFO")
